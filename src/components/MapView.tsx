...
export default function MapView({ 
 ...
 showToast(lang === 'ar' ? ' تم إخفاء المهمة المكتملة الوصول بنجاح!' : ' Arrived quest hidden successfully!');
 };
 ...
 useEffect(() => {
 if (navigatingQuest && navigatingQuest.id !== prevNavIdRef.current) {
 prevNavIdRef.current = navigatingQuest.id;
 if (userLoc) {
 setNavStartLoc({ lat: userLoc.lat, lng: userLoc.lng });
 }
 } else if (!navigatingQuest) {
 prevNavIdRef.current = null;
 }
 }, [navigatingQuest, userLoc]);
 ...
 const getDistanceToPolyline = useCallback((lat: number, lng: number, points: [number, number][]) => {
 ...
 const getClosestPointIndex = useCallback((lat: number, lng: number, points: [number, number][]) => {
 ...
 lastRouteStartRef.current = { lat: start.lat, lng: start.lng };
 ...
 if (q.status === 'open' || q.status === 'applications' || q.status === 'pending_verification') return true;
 ...
 const handleOffline = () => {
 setIsGpsLost(true);
 showToast(lang === 'ar' ? ' انقطع الاتصال بالشبكة والـ GPS' : ' Network / GPS signal lost');
 };
 ...
 const remainingPointsAhead = lockedRoutePoints.slice(closestIdx);
 if (remainingPointsAhead.length > 0) {
 let totalMeters = calculateDistanceMeters(
 userLoc.lat,
 userLoc.lng,
 remainingPointsAhead[0][0],
 remainingPointsAhead[0][1]
 );
 for (let i = 0; i < remainingPointsAhead.length - 1; i++) {
 totalMeters += calculateDistanceMeters(
 remainingPointsAhead[i][0],
 remainingPointsAhead[i][1],
 remainingPointsAhead[i + 1][0],
 remainingPointsAhead[i + 1][1]
 );
 }
 ...
 return calculateDistanceKm(navCoords.lat, navCoords.lng);
 }, [navigatingQuest, userLoc, lockedRoutePoints, getQuestCoords, calculateDistanceKm, getClosestPointIndex, calculateDistanceMeters]);
 ...
 <span>{lang === 'ar' ? ' الموقع مخفي حتى قبول الحجز' : ' Hidden until booked'}</span>
 </>
 )}
 </p>
 </div>
 ...
 // Bypassed dead code
 const tokenAmount = calculateBookingFee(showDetailedSheet.cashReward, showDetailedSheet.questType);
 ...
 <h3 className="text-2xl sm:text-3xl font-black text-sky-500 dark:text-sky-400 leading-snug tracking-tight text-start mt-1 pr-10">
 ...
 <p className="text-xs sm:text-sm text-slate-700 dark:text-slate-200 leading-relaxed font-medium whitespace-pre-line text-start mt-1">
 ...
 {showDetailedSheet.cashReward} <span className="text-xs font-sans text-gray-300 font-semibold">{lang === 'ar' ? 'د.ج' : 'DZD'}</span>
 </span>
 </div>
 ...
 isBookedByMe 
 ? 'bg-[#38BDF8] text-[#0F172A] shadow-[#38BDF8]/10' 
