...
export default function App() {
 ...
 showToast('ℹ تم إلغاء أو إغلاق نافذة الدخول بـ Google.');
 return;
 }
 ...
 showToast(' تم حظر النافذة المنبثقة بواسطة المتصفح. يرجى فتح التطبيق في تبويب جديد أو السماح بالنوافذ المنبثقة.');
 }
 } else {
 ...
 showToast(' فشل تسجيل الدخول: ' + errMsg);
 }
 }
 };
 ...
 try {
 await signOut(auth);
 showToast('ℹ تم تسجيل الخروج من حسابك السحابي.');
 } catch (e: any) {
 ...
 showToast(userProfile.language === 'ar' 
 ? ' لا يمكنك التقديم على كويست قمت بنشره بنفسك!' 
 : ' You cannot apply to your own quests!'
 );
 return;
 }
 ...
 if (alreadyApplied) {
 showToast(userProfile.language === 'ar' 
 ? ' لقد قمت بالتقديم على هذا الكويست مسبقاً.' 
 ...
 const isLongTerm = targetQuest.questType === 'long_term';

 // Calculate implicit platform fee: 5% of cashReward for quick tasks, 0 for long-term jobs
 const fee = isLongTerm ? 0 : calculateBookingFee(targetQuest.cashReward, targetQuest.questType);
 ...
 targetQuest.jobApplicants?.find(app => app.applicantId === applicantId);

 if (isLongTerm) {
 // Create job contract for long-term employment
 const contractId = `contract_${Date.now()}_${questId}`;
 const newContract = {
 id: contractId,
 jobId: questId,
 jobTitle: targetQuest.title,
 employerId: targetQuest.creatorId,
 ...
 salary: targetQuest.cashReward,
 salaryPeriod: targetQuest.salaryPeriod || 'monthly',
 employmentType: targetQuest.employmentType || 'full_time',
 ...
 if (auth.currentUser) {
 try {
 const qToUpdate = updatedQuests.find(q => q.id === questId);
 if (qToUpdate) {
 await setDoc(doc(db, 'quests', questId), cleanData(qToUpdate));
 }
 } catch (err) {
 console.error("Could not sync long-term job status to DB:", err);
 }
 }

 addNotification(
 applicantId,
 ` تهانينا! تم توظيفك لوظيفة "${targetQuest.title}". تم توثيق عقد العمل بنجاح!`,
 questId,
 'approved'
 );
 ...
 applicantId,
 userProfile.language === 'ar' ? ' تم توظيفك بنجاح!' : ' You have been Hired!',
 userProfile.language === 'ar'
 ? `تم توظيفك لوظيفة "${targetQuest.title}". افتح العقد للتفاصيل.`
 : `You have been hired for "${targetQuest.title}". Click to view contract.`,
 { questId }
 );

 showToast(
 userProfile.language === 'ar'
 ? ' تم قبول طلب العمل وتوثيق العقد بنجاح!'
 : ' Job application accepted and contract created successfully!'
 );
 return;
 }

 // Support multi-slot hiring for quick tasks
 ...
 showToast(userProfile.language === 'ar' ? ' هذا العامل معين بالفعل لهذه المهمة!' : ' This worker is already assigned to this quest!');
 return;
 }
 ...
 addNotification(
 applicantId,
 `تم قبول طلبك لمهمة ${targetQuest.title}! افتح الخريطة لبدء التوجيه الميداني الحركي `,
 questId,
 'approved'
 );
 sendPushNotification(
 ...
 const handleRejectApplicant = (questId: string, applicantId: string) => {
 const targetQuest = quests.find(q => q.id === questId);
 if (!targetQuest) return;
 ...
 return {
 ...q,
 applicants: q.applicants?.filter(a => a.userId !== applicantId)
 };
 }
 return q;
 });
 ...
 applicantId,
 userProfile.language === 'ar' 
 ? `عذراً، لم يتم قبول طلبك لمهمة "${targetQuest.title}".` 
 : `Sorry, your request for "${targetQuest.title}" was rejected.`,
 questId,
 'rejected'
 );

 showToast(userProfile.language === 'ar' ? 'تم رفض المتقدم بنجاح' : 'Applicant rejected successfully');
 };
 ...
 if (!isRunner) {
 showToast(userProfile.language === 'ar'
 ? ' لا يمكنك تأكيد الوصول لمهمة أنت صاحبها أو لست الكابتن المعين لها!'
 : ' You cannot confirm arrival for a quest you created or are not assigned to as runner!'
 );
 return;
 }
 ...
 const questTokensCost = targetQuest.bookingFeeDA || targetQuest.requiredDA || calculateBookingFee(targetQuest.cashReward || 1000, targetQuest.questType);
 ...
 showToast(' إنجاز مذهل: لقد فتحت شارة بطل الحي المحنك!');
 }
 ...
 bookingFeeDA: calculateBookingFee(newQuestData.cashReward || 1000, newQuestData.questType),
 ...
 questType: newQuestData.questType || 'quick',
 salaryPeriod: newQuestData.salaryPeriod,
 employmentType: newQuestData.employmentType,
 durationType: newQuestData.durationType,
 startDate: newQuestData.startDate,
 endDate: newQuestData.endDate,
 requiredSkills: newQuestData.requiredSkills,
 };
 ...
 const handleRequestEndWork = async (questId: string, reason?: string) => {
    ...
    const recipientId = isCreator ? workerId : targetQuest.creatorId;
    ...
    if (auth.currentUser) {
      try {
        await setDoc(doc(db, "quests", questId), cleanData(updatedQuest));
      } catch (err) {
        console.error("Could not update quest ending status in DB:", err);
      }
      ...
      if (targetQuest.contractId) {
        try {
          ...
          console.warn("Could not update contract status in DB:", err);
        }
      }
    }
  ...
  const handleConfirmEndWork = async (questId: string) => {
    if (!userProfile) return;
    const targetQuest = quests.find(q => q.id === questId);
    if (!targetQuest) return;
    ...
    if (auth.currentUser) {
      try {
        ...
        console.error("Could not archive completed long-term quest in DB:", err);
      }
      ...
      if (targetQuest.contractId) {
        try {
          ...
          console.warn("Could not terminate contract in DB:", err);
        }
      }
    }

    if (recipientId) {
      const msgAr = `تم تأكيد إنهاء العمل في "${targetQuest.title}" بنجاح ونقل العقد إلى الأرشيف.`;
      const msgEn = `Work termination for "${targetQuest.title}" confirmed and archived.`;

      addNotification(recipientId, userProfile.language === "ar" ? msgAr : msgEn, questId, "approved");
      sendPushNotification(
        recipientId,
        userProfile.language === "ar" ? "تم تأكيد إنهاء العمل" : "Work Termination Confirmed",
        userProfile.language === "ar" ? msgAr : msgEn,
        { questId }
      );
    }
    ...
    showToast(
      userProfile.language === "ar"
        ? "تم إنهاء العمل بنجاح ونقل الوظيفة إلى الأرشيف."
        : "Work terminated successfully and contract archived."
    );
  };

  const handleRejectEndWork = async (questId: string) => {
    if (!userProfile) return;
    const targetQuest = quests.find(q => q.id === questId);
    if (!targetQuest) return;
    ...
    if (auth.currentUser) {
      try {
        ...
        console.error("Could not update quest status in DB:", err);
      }
    }

    if (recipientId) {
      const msgAr = `أشار ${userProfile.name} إلى أن العمل في "${targetQuest.title}" ما زال مستمراً.`;
      const msgEn = `${userProfile.name} indicated that work for "${targetQuest.title}" is still ongoing.`;

      addNotification(recipientId, userProfile.language === "ar" ? msgAr : msgEn, questId, "warning");
      sendPushNotification(
        recipientId,
        userProfile.language === "ar" ? "العمل ما زال مستمراً" : "Work Still Ongoing",
        userProfile.language === "ar" ? msgAr : msgEn,
        { questId }
      );
    }
    ...
    showToast(
      userProfile.language === "ar"
        ? "تم إعلام الطرف الآخر بأن العمل ما زال مستمراً."
        : "The other party was notified that work is still ongoing."
    );
  };
 ...
 if (!alreadyClaimed) {
 showToast('Approved user KYC identity! Extra 700 DA bonus & Verified Badge unlocked successfully! ');
 } else {
 showToast('Approved user KYC identity! Verified Badge unlocked successfully! ');
 }
 } else {
 ...
 className="w-full bg-[#FF3B7C] hover:bg-[#FF3B7C]/90 text-white font-extrabold text-xs py-3.5 rounded-xl transition duration-200 flex items-center justify-center gap-2 shadow-lg shadow-[#FF3B7C]/15 cursor-pointer"
 >
 ...
 showToast(userProfile?.language === 'ar' ? ' لا يمكنك فتح الخريطة أو تتبع الموقع حتى تقوم بحجز المهمة أولاً!' : ' You cannot open the map or view location until you book the quest first!');
 }
 }}
 ...
 onExtendActiveContract={handleExtendActiveContract}
  onRequestEndWork={handleRequestEndWork}
  onConfirmEndWork={handleConfirmEndWork}
  onRejectEndWork={handleRejectEndWork}
 showToast={showToast}
 />
 ) : (
 <>
 ...
 <li>{userProfile.language === 'ar' ? 'مراسلة الدعم الفني لتقديم التماس فك التجميد.' : 'Contact Technical Support immediately to submit an appeal.'}</li>
 ...
 onActiveChatChange={setIsInsideChat}
 onOpenChat={(chatId) => {
 ...
 {userProfile?.language === 'ar' 
 ? 'رصيد الاستخدام غير كافٍ لحجز المهمة! ' 
 ...
 >
 {userProfile?.language === 'ar' ? ' توثيق هويتي الآن مجاناً (+700 د.ج)' : ' Verify Identity for Free (+700 DA)'}
 </button>
 ...
 {userProfile?.language === 'ar' ? 'تحديد الموقع معطل' : 'Location Services Disabled'}
 </h3>
 ...
 <span className="absolute inline-flex h-20 w-20 rounded-full bg-[#38BDF8] opacity-20 animate-ping"></span>
 <span className="absolute inline-flex h-16 w-16 rounded-full bg-[#38BDF8] opacity-30 animate-pulse"></span>
 <div className="relative w-16 h-16 bg-[#38BDF8] rounded-full flex items-center justify-center shadow-lg border-2 border-white">
 <span className="text-3xl"></span>
 </div>
 </div>
 ...
 {userProfile?.language === 'ar' ? 'تنبيه وصول ' : 'LIVE ARRIVAL ALERT '}
 </span>
 ...
 const runnerId = isCreator ? helperId : (userProfile?.id || helperId);
 const approvedChatId = `${quest.id}_${quest.creatorId}_${runnerId}`;
 window.dispatchEvent(new CustomEvent('open-chat', {
 detail: {
 chatId: approvedChatId,
 questTitle: quest.title,
 recipientName: isCreator ? (quest.helperName || (userProfile?.language === 'ar' ? 'المساعد الميداني' : 'Field Assistant')) : quest.creatorName,
 recipientAvatar: isCreator ? (`https://api.dicebear.com/7.x/initials/svg?seed=${quest.helperName || 'Runner'}&backgroundColor=f43f5e`) : quest.creatorAvatar
 }
 }));
 }}
