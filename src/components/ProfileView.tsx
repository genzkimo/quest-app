...
export default function ProfileView({ 
 ...
 showToast(lang === 'ar'
 ? (newVal ? ' تم تفعيل وضع تسريع الأجهزة الضعيفة! تم تقليل المؤثرات البصرية والخلفيات المضببة لضمان سلاسة فائقة.' : ' تم العودة للوضع الرسومي الكامل.')
 : (newVal ? ' Performance speed mode activated! Animations and blurs reduced for optimal FPS.' : ' Returned to full quality graphics mode.')
 );
 };
 ...
 showToast(lang === 'ar' ? ' لقد وصلت للحد الأقصى المسموح به (10 صور) في معرض الأعمال!' : ' You have reached the maximum limit of 10 photos in your portfolio!');
 return;
 }
 ...
 if (portfolioPhotos.length >= 10) {
 showToast(lang === 'ar' ? ' لقد وصلت للحد الأقصى المسموح به (10 صور)!' : ' Maximum of 10 photos reached!');
 return;
 }
 ...
 console.error(err);
 setPortfolioUploading(false);
 showToast(' Error processing chosen portfolio image file');
 }
 };
 ...
 console.error(err);
 showToast(' Error loading gallery image');
 }
 };
 ...
 console.error(err);
 setKycUploadingFront(false);
 showToast(' Failed to compress ID Front Card screenshot');
 }
 };
 ...
 console.error(err);
 setKycUploadingBack(false);
 showToast(' Failed to compress ID Back Card screenshot');
 }
 };
 ...
 showToast(lang === 'ar' ? ' يرجى تعبئة كافة الحقول وبصمات الصور!' : ' Fill all fields and upload physical screenshots.');
 return;
 }
...
const response = await fetch(`${API_BASE_URL}/api/kyc/verify`, {
 ...
 showToast(lang === 'ar' ? ' حدث خطأ رقمي مؤقت. تم حفظ طلبك وإرساله للمراجعة اليدوية وبدء فترة الانتظار (15 دقيقة).' : ' Temporary technical error. Saved for manual review & 15-minute cooldown applied.');
 } finally {
 ...
 showToast(lang === 'ar' 
 ? ' تم وبنجاح كامل حذف كافة مستندات الهوية وسجلات الحساب والامتثال لحق النسيان!' 
 : ' Successfully expunged NID records and deleted the account. Compliance fulfilled!');
 ...
 showToast(lang === 'ar' ? ' حدث خطأ أثناء حذف الحساب: ' + err.message : ' Error complying with account deletion: ' + err.message);
 }
 };
 ...
 console.error(err);
 setRefillReceiptUploading(false);
 showToast(' Failed to load or compress receipt image');
 }
 };
 ...
 showToast(lang === 'ar' ? ' يرجى ملء كافة معلومات البطاقة الائتمانية!' : ' Please fill out all credit card fields!');
 return;
 }
 if (cardNumber.trim().replace(/\s/g, '').length < 12) {
 showToast(lang === 'ar' ? ' رقم البطاقة غير صالح!' : ' Card number is invalid!');
 return;
 }
 } else {
 if (!paypalPayerEmail.trim() || !paypalPayerEmail.includes('@')) {
 showToast(lang === 'ar' ? ' يرجى إدخال بريد PayPal الإلكتروني صالح!' : ' Please enter a valid PayPal email address!');
 return;
 }
 }

 if (refillAmountUSD <= 0) {
 showToast(lang === 'ar' ? ' يرجى إدخال قيمة شحن صالحة بالدولار!' : ' Please enter a valid USD amount!');
 return;
 }
...
const response = await fetch(`${API_BASE_URL}/api/wallet/initiate-paypal-refill`, {
 ...
 showToast(lang === 'ar'
 ? ' حدث عطل فني في بوابة الدفع. تم حفظ طلب الشحن وتوجيهه للمراجعة اليدوية لضمان شحن رصيدك.'
 : ' Payment gateway error. Your recharge request has been saved and routed for manual administrative review.'
 );
 } catch (dbErr) {
 ...
 if (refillAmount <= 0) {
 showToast(lang === 'ar' ? ' يرجى إدخال مبلغ شحن صالح!' : ' Please enter a valid recharge amount!');
 return;
 }
 if (!refillReference.trim()) {
 showToast(lang === 'ar' ? ' يرجى كتابة رقم العملية / الوصل المرجعي!' : ' Reference number is required!');
 return;
 }
 if (!refillDate) {
 showToast(lang === 'ar' ? ' يرجى تحديد تاريخ العملية!' : ' Transaction date is required!');
 return;
 }
 if (!refillReceiptBase64) {
 showToast(lang === 'ar' ? ' يرجى إرفاق صورة أو لقطة شاشة لوصل الدفع!' : ' Please upload a crop screenshot or photo of payment receipt!');
 return;
 }
...
const response = await fetch(`${API_BASE_URL}/api/wallet/refill-manual`, {
 ...
 cdMs > 0
 ? (lang === 'ar' ? ` تم رفض عملية الشحن التلقائية. تم توقيف الزر مؤقتاً لمدة (${cdLabel}) لمكافحة الاحتيال.` : ` Verification failed! Button locked for (${cdLabel}).`)
 : (lang === 'ar' ? ` تم رفض عملية الشحن التلقائية. (محاولة ${nextRefillAttempts} من 3)` : ` Verification failed. (Attempt ${nextRefillAttempts} of 3)`)
 );
 showToast(errMsg);
 ...
 showToast(lang === 'ar' ? ' حدث خطأ رقمي مؤقت. تم حفظ طلبك وتوجيهه للمراجعة اليدوية لضمان حقك.' : ' Temporary network error. Your payment request has been saved and routed for manual review.');
 ...
 showToast(lang === 'ar' ? ' فشل الاتصال بنظام مكافحة الاحتيال وقاعدة البيانات، يرجى المحاولة لاحقاً.' : ' Failed to connect to the verification server or database. Please try again.');
 }
 } finally {
 setRefillLoading(false);
 }
...
const response = await fetch(`${API_BASE_URL}/api/wallet/verify-otp-refill`, {
 ...
 showToast(lang === 'ar'
 ? ' فشل تأكيد الرمز الرقمي. تم توجيه طلب الشحن الخاص بك للمراجعة اليدوية لضمان شحن رصيدك.'
 : ' Verification failed. Your recharge request has been saved and routed for manual administrative review.'
 );
 } catch (dbErr) {
 ...
 <span className="text-[9px] bg-[#FF3B7C] text-white font-black px-1.5 py-0.5 rounded uppercase">bonus </span>
 ...
 <div className="text-right">
 <h4 className="text-xs font-black text-[#1F2A44]">
 {lang === 'ar' ? ' الإعدادات العامة' : ' General System Preferences'}
 </h4>
 ...
 <p className="text-[10px] text-gray-455 font-semibold mt-0.5">
 {lang === 'ar' ? 'حل مشكلات التطبيق، تتبع كشوف حساباتك والتواصل مع فريقنا' : 'Submit feedback, ask questions or resolve errors'}
 </p>
 </div>
 </div>
 <ChevronRight className={`w-4 h-4 text-gray-400 transition-transform ${isRtl ? 'rotate-180 group-hover:-translate-x-1' : 'group-hover:translate-x-1'}`} />
 ...
 <span className={`w-2 h-2 rounded-full ${authenticatedUser ? 'bg-sky-500 animate-pulse' : 'bg-rose-400'}`}></span>
 ...
 title={lang === 'ar' ? 'التقاط صورة سيلفي بالكاميرا' : 'Take selfie with camera'}
 >
 ...
 <span>{lang === 'ar' ? 'منطقة الخطر وحماية الخصوصية ' : 'Danger Zone & Privacy Protection '}</span>
 </h4>
 ...
 <span className="text-[10px] font-black uppercase text-[#4FC3F7] tracking-wider block text-center">Verified Operator Credentials </span>
 ...
 {/* Secure Data Access & Privacy Disclosure Banner */}
 ...
 kycAiResult.status === 'APPROVED' ? 'bg-sky-500 animate-pulse' : kycAiResult.status === 'SUSPICIOUS' ? 'bg-amber-400 animate-pulse' : 'bg-rose-500'
 }`}></span>
 ...
 {lang === 'ar' ? 'رصيد الاستخدام' : 'Usage Balance'}
 </span>
 <span className="text-[10px] text-gray-450 font-semibold block leading-relaxed">
 {lang === 'ar' ? 'يُستخدم لدفع رسوم نشر وحجز المهام.' : 'Used for booking & publishing fees.'}
 </span>
 </div>
 ...
 <div className="flex items-center justify-between gap-3 w-full border border-sky-500/20 bg-sky-50/70 p-3 rounded-2xl text-right">
 <div className="flex items-center gap-2">
 <span className="inline-block p-1.5 bg-sky-100 rounded-full text-sky-600 font-black"></span>
 <div className="text-right">
 <span className="text-[10px] font-black text-sky-800 block leading-tight">
 {lang === 'ar' ? 'تم مرفق وصل الدفع بنجاح!' : 'Receipt attached successfully!'}
 </span>
 <span className="text-[8px] text-sky-600 block truncate max-w-[150px] font-mono">
 {refillReceiptFileName}
 </span>
 </div>
 </div>
 <div className="flex gap-1.5">
 <label htmlFor="receipt-proof-camera-file" className="px-2 py-1 bg-sky-600 text-white text-[9px] font-black rounded-lg cursor-pointer">
 ...
 <span className="text-[10px] font-black text-slate-800 block">
 {lang === 'ar' ? ' لقطة من المعرض' : ' Gallery Screen'}
 </span>
 ...
 verificationResult.status === 'APPROVED' ? 'bg-sky-500 animate-pulse' : verificationResult.status === 'REJECTED' ? 'bg-rose-500' : 'bg-amber-500 animate-pulse'
 }`}></span>
 ...
 req.status === 'approved' ? 'bg-sky-100 text-sky-800' :
 ...
 {lang === 'ar' ? ' سبب عدم القبول / توضيح النظام والذكاء الاصطناعي:' : ' Rejection Reason / AI Explanation:'}
 </span>
 ...
 {lang === 'ar' 
 ? ' خطوة هامة: يرجى النقر أدناه لإتمام عملية الدفع الحقيقية والخصم عبر بوابة PayPal الآمنة أولاً:' 
 : ' Crucial Step: Please click below to complete the real PayPal payment and authorize the charge first:'}
 </p>
 <a 
 ...
 >
 <span>{lang === 'ar' ? ' ادفع الآن عبر PayPal' : ' Pay Now with PayPal'}</span>
 </a>
 </div>
 )}
 ...
 <div className={`space-y-0.5 ${isRtl ? 'text-right' : 'text-left'}`}>
 <h5 className="font-extrabold text-xs text-[#1F2A44]">
 {lang === 'ar' ? 'الشروط والسياسات القانونية ' : 'Legal Policies & Terms '}
 </h5>
 <p className="text-[9px] text-gray-400 font-semibold">
 ...
 ticket.status === 'resolved' ? 'bg-sky-100 text-sky-800' : 'bg-amber-100 text-amber-800'
 }`}>
 ...
 <div className="p-3 bg-sky-50 text-sky-800 rounded-xl border border-sky-100 text-center text-[10px] font-extrabold">
 ...
 <span className={`text-[8px] font-black px-1.5 py-0.5 rounded ${
 t.status === 'resolved' ? 'bg-sky-100 text-sky-800' : 'bg-amber-100 text-amber-800'
 }`}>
 ...
 if (isClaimed) {
 cardStyles = "bg-sky-50 border border-sky-200 text-sky-600";
 rewardTextHighlight = "text-sky-600";
 } else if (isActive) {
 ...
 {isClaimed ? (
 <div className="w-4 h-4 bg-sky-500 text-white rounded-full flex items-center justify-center mx-auto text-[8px] font-black">
 ...
 <div className="bg-sky-50 border border-sky-500 p-3 rounded-xl space-y-1 text-center">
 <span className="text-[9px] text-sky-800 font-black block">
 ...
 <div className="text-[9px] font-extrabold text-sky-800 mt-1">
 ...
 */}
 ...
 if (portfolioPhotos.length >= 10) {
 e.preventDefault();
 showToast(lang === 'ar' ? ' لقد وصلت للحد الأقصى المسموح به (10 صور) في معرض الأعمال!' : ' Max limit reached (10 photos)!');
 return;
 }
 playCameraShutter(audioEffectsEnabled);
 }}
 ...
 {lang === 'ar' 
 ? ' لقد وصلت للحد الأقصى المسموح به (10 صور) لمعرض أعمالك!' 
 : ' You have reached the maximum allowed limit of 10 photos in your portfolio!'}
 </div>
 )}
 ...
 showToast(lang === 'ar' ? ' تم إزالة الصورة من البورتفوليو!' : ' Removed portfolio photo!');
 }}
 ...
 {lang === 'ar' ? 'تأكيد الحذف النهائي والأبدي؟ ' : 'Confirm Permanent Deletion?'}
 </h3>
