...
const API_BASE_URL = import.meta.env.VITE_API_URL || 'https://quest-app-prod.onrender.com';
...
export default function ProfileView({ 
  ...
  const executeRefill = async (e: React.FormEvent) => {
    ...
    if (refillPaymentMethod === 'paypal') {
      ...
      try {
        const response = await fetch('/api/wallet/initiate-paypal-refill', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
            userId: authenticatedUser?.uid || 'user-current',
    ...
    try {
      const response = await fetch('/api/wallet/refill-manual', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          amount: refillAmount,
  ...
  const handleVerifyOtpAndCredit = async () => {
    ...
    try {
      const response = await fetch('/api/wallet/verify-otp-refill', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          userId: authenticatedUser?.uid || 'user-current',
