...
export interface JobContract {
 ...
 startDate: string;
 endDate?: string | null;
 contractType: 'fixed' | 'ongoing' | 'fixed_term' | 'indefinite';
 salary: number;
 ...
 createdAt: string;
 terminatedAt?: string;
 terminatedBy?: 'employer' | 'employee';
...
export interface Quest {
 ...
 status: 'open' | 'booked' | 'active' | 'arrived' | 'pending_verification' | 'completed' | 'disputed' | 'cancelled_by_timeout' | 'cancelled' | 'stale_cleared' | 'applications' | 'hired' | 'active_employment' | 'ending' | 'terminated' | 'expired' | 'archived' | 'withdrawn';
 ...
 extensionApprovedBy?: string | null;
  ...
  terminatedBy?: 'employer' | 'employee' | 'mutual' | string;
 creatorId: string;
 creatorName: string;
 creatorPhone: string;
 ...
 questType?: 'quick' | 'long_term'; // Distinguish between quick task and long-term job
 salaryPeriod?: 'monthly' | 'weekly' | 'daily' | 'hourly'; // Salary period for long-term jobs
 employmentType?: 'full_time' | 'part_time'; // Full time or part time
 durationType?: 'fixed' | 'ongoing'; // Fixed duration or continuous/ongoing
 startDate?: string; // Job start date
 endDate?: string; // Job end date (if fixed duration)
...
export interface UserProfile {
 ...
 jobSearchPreference?: 'quick' | 'long_term' | 'both'; // Profile preference for opportunity types
}
