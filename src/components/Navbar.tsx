...
export default function Navbar({ 
 ...
 {unreadNotificationsCount}
  ...
  {/* Integrated Internet Connection Status Bar */}
  <AnimatePresence>
    {showConnectionBar && activeConnectionStatus !== 'online' && (
      <motion.div
        initial={{ opacity: 0, height: 0 }}
        animate={{ opacity: 1, height: "auto" }}
        exit={{ opacity: 0, height: 0 }}
        transition={{ duration: 0.18, ease: "easeOut" }}
        className={`px-4 py-1 flex items-center justify-between text-[11px] font-semibold backdrop-blur-xl border-b transition-all shadow-2xs ${
          activeConnectionStatus === "offline"
            ? "bg-rose-50/80 border-rose-200/70 text-rose-800 backdrop-blur-xl"
            : activeConnectionStatus === "weak"
            ? "bg-amber-50/80 border-amber-200/70 text-amber-800 backdrop-blur-xl"
            : "bg-white/80 border-slate-200/60 text-slate-700 backdrop-blur-xl"
        }`}
      >
        <div className="flex items-center gap-2 text-[11px] font-bold mx-auto">
          <span className={`w-2 h-2 rounded-full ${
            activeConnectionStatus === "offline" ? "bg-rose-500" : activeConnectionStatus === "weak" ? "bg-amber-500" : "bg-emerald-500"
          }`} />
          <span>
            {activeConnectionStatus === "offline" ? (
              lang === "ar" ? "لا يوجد اتصال بالإنترنت - يرجى التحقق من الشبكة" : "No internet connection - please check network"
            ) : (
              lang === "ar" ? "الاتصال بالإنترنت ضعيف - قد تواجه بعض البطء" : "Connection is weak - performance may slow"
            )}
          </span>
        </div>
        {onCloseConnectionBar && (
          <button 
            ...
            className="text-slate-400 hover:text-slate-600 p-0.5 cursor-pointer text-xs font-bold leading-none shrink-0"
