...
export default function LeaderboardView({ 
 ...
 setToastMessage(lang === 'ar' ? ` رصيد نقاط غير كافٍ لفتح [${badge.title}]` : ` Insufficient points to unlock [${badge.title}]`);
 setTimeout(() => setToastMessage(null), 3500);
 return;
 }
 
 onUnlockBadge(badge.id, cost);
 setToastMessage(lang === 'ar' ? ` تم فتح شارة [${badge.title}] بنجاح!` : ` Badge [${badge.title}] is now unlocked in your gallery!`);
 setTimeout(() => setToastMessage(null), 3500);
 };
 ...
 */}
 ...
 isEligibleToClaim ? 'bg-sky-500' : 'bg-[#FF3B7C]'
 }`}
 ...
 className="bg-sky-600 hover:bg-sky-700 text-white font-black text-xs px-4 py-2 rounded-xl cursor-pointer transition-all"
 >
 Claim Reward Payout! 
 </button>
 ) : (
 ...
 hasUnlocked ? 'bg-amber-500' : hasEnoughPoints ? 'bg-sky-500' : 'bg-red-500'
 }`}
 ...
 <span className={`font-black uppercase ${hasUnlocked ? 'text-sky-600' : 'text-gray-400'}`}>
